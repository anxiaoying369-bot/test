...
* **顶部操控区**：包含一个优雅的深色输入框（带 Focus 时的蓝色外发光边框），提示词："输入搞钱关键词 (如: 抹茶欧包)..."。右侧是一个亮蓝色的实体按钮 `[ + 投放新任务 ]`。
* **中部队列列表**（垂直滚动，支持排队、插队可视化）：
    * **执行中卡片（居顶）**：边框呈淡淡的绿色呼吸晕染效果。带有进度条和动态变化的小图标。
    * **高优先级插队卡片**：如微信回复，带有一个闪烁的红色惊叹号。排在所有普通生成任务前面。
    * **等待中卡片队列**：暗灰色卡片，右上角标有黄色微标 `排队中`。
