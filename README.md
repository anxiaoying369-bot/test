...
| 数据存储 | 本地 JSON、JSONL、SQLite/采集模块数据文件 |
...
├── tsconfig.json
├── src/
│   ├── App.vue                         # 主界面：导航、账号管理、页面容器
│   ├── main.ts                         # Vue 入口
│   ├── assets/styles.css               # Tailwind CSS v4 样式入口
│   ├── components/
│   │   ├── ScraperView.vue             # 评论采集任务界面
│   │   ├── ResultsView.vue             # 采集结果查看与 AI 分析原型
│   │   └── LiveMonitorView.vue         # 直播间实时监控界面
│   └── lib/utils.ts                    # className 合并工具
├── src-tauri/
│   ├── Cargo.toml
│   ├── tauri.conf.json
│   └── src/lib.rs                      # Tauri 命令、进程管理、本地数据路径
└── scripts/
    ├── douyin_login.py                 # 抖音登录，DrissionPage + Chrome CDP
    ├── verify_account.py               # Cookie 有效性验证，L1 + L2 CDP
    ├── douyin_scraper.py               # 评论/作品/回复采集桥接脚本
    ├── query_data.py                   # 查询采集结果供前端展示
    ├── open_video.py                   # 注入 Cookie 后打开抖音作品
    ├── douyin_live_monitor.py          # 直播监控桥接脚本
    ├── funcall.py
    ├── validator.py
    ├── DouyinComment/                  # 抖音评论采集子模块
    └── DouyinBarrage/                  # 抖音直播弹幕监控模块
```
