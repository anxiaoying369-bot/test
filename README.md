...
- **一键发布与定时排期**：视频工作室成片可直接发布到抖音创作者中心，支持「立即 / 定时」两种时机与**多账号矩阵分发**（同一视频一次提交、各账号各成一条任务）；后台 tokio 调度器到点自动发布，复用账号管理已保存的登录 Cookie，无需重复扫码。
...
| 发布排期 | `PublishView` | 成片一键发布到抖音创作者中心，支持定时排期与多账号矩阵分发（DrissionPage 驱动，复用已登录 cookie） |
