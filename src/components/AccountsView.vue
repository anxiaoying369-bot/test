...
async function confirmDeleteAccount(account: any) {
  confirmDeleteKey.value = null;
  try {
    await invoke('delete_account', { platform: account.platform, name: account.name });
...
async function confirmAndSave() {
  ...
  try {
    const account: any = await invoke('finish_login', { sessionId: sessionId.value, accountName: name });
...
<template>
  <main class="flex flex-col flex-1 min-w-0 h-full bg-gray-950 p-6 overflow-y-auto">
    ...
    <div v-if="isLoginModalOpen" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center">
      <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 w-[360px] shadow-2xl relative">
        ...
        <h3 class="text-lg font-bold mb-4 text-center">♪ 抖音登录</h3>
