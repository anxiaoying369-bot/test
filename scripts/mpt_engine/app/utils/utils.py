...
def storage_dir(sub_dir: str = "", create: bool = False):
    # AutoCast AI 改造：任务产物（音频/字幕/素材缓存/成片）写到桌面端的数据目录，
    # 而不是引擎包目录。Rust 侧通过 MPT_STORAGE_DIR 注入该目录；未注入时回退到
    # 引擎包内的 storage（便于命令行单测）。
    base = os.environ.get("MPT_STORAGE_DIR") or os.path.join(root_dir(), "storage")
    d = base
    if sub_dir:
