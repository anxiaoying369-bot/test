...
# Node.js 环境补丁：定义缺失的全局变量，防止 ReferenceError
_NODE_PATCH = '''
...
def _ensure_sign_js():
    ...
    with _sign_init_lock:
        ...
        # 复制到临时目录并增加补丁和 wrapper
        tmp_dir = os.path.join(tempfile.gettempdir(), 'douyin_sign')
        os.makedirs(tmp_dir, exist_ok=True)
        tmp_path = os.path.join(tmp_dir, 'sign.js')

        # 先写入全局变量补丁，再写入原始内容，最后写入 stdin wrapper
        with open(tmp_path, 'w', encoding='utf-8') as f:
            f.write(_NODE_PATCH)
            f.write(content)
            if '// stdin 接口 - 供 Python subprocess 调用' not in content:
                f.write(_STDIN_WRAPPER)

        _SIGN_JS_PATCHED = tmp_path
        logger.debug(f"[签名] 已复制并修复 sign.js 到临时目录: {tmp_path}")
        return _SIGN_JS_PATCHED
