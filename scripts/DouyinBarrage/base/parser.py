...
def parse_gift_msg(payload, enable_outputs=None):
    ...
    # 优先使用 name，无则用 describe
    gname = gift.name or gift.describe or "未知礼物"
...
def parse_like_msg(payload, enable_outputs=None):
    ...
    uid = get_user_id(user)
    # 忽略系统自动发送的 0 赞消息
    if msg.count == 0 and msg.total == 0:
        return []
    return [{
        'type': 'like',
...
def parse_social_msg(payload, enable_outputs=None):
    ...
    # 抖音 SocialMessage action 定义：
    # 1: 关注了主播
    # 2: 分享了直播间
    # 3: 进入了直播间 (通常是高等级用户或粉丝)
    # 5: 邀请了好友
    
    action_map = {1: "关注了主播", 2: "分享了直播间", 3: "进入了直播间", 5: "邀请了好友"}
    if msg.action not in action_map:
        return []
        
    action_text = action_map[msg.action]
    
    # 如果是进场，我们把它映射为 member 类型，以便 UI 过滤器生效
    dtype = 'member' if msg.action == 3 else 'social'
    
    follow = f"(第{msg.follow_count}个关注)" if msg.follow_count and msg.action == 1 else ""
    return [{
        'type': dtype,
