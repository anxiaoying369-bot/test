...
async def run_scrape(cookie_str: str, sec_uid: str, scrape_type: str,
    ...
    try:
        ...
        if scrape_type in ('video', 'all'):
            ...
            # 用独立实例预取用户信息，避免污染后续 run() 的分页游标
            try:
                info_service = VideoService(sec_uid, cookie_str)
                raw_videos = await info_service.fetch(page_size=1, limit=1)
                if raw_videos:
                ...
                _log(f"[SCRAPER] 预取用户信息失败（非致命）: {e}")

            # 用全新实例执行正式采集，确保 limit 从第 0 条开始计数
            service = VideoService(sec_uid, cookie_str)
            stats = await service.run(delay=delay, limit=limit, incremental=incremental)
            all_stats['video'] = stats
            _log(f"[SCRAPER] 作品采集完成: {stats}")

            current_progress = 25 if scrape_type == 'all' else 100
        ...
        if scrape_type in ('comment', 'all'):
            ...
            current_progress = 50 if scrape_type == 'all' else 100
            progress.update(progress=current_progress, stats=all_stats)
        ...
        if scrape_type in ('reply', 'all'):
            ...
            current_progress = 75 if scrape_type == 'all' else 100
            progress.update(progress=current_progress, stats=all_stats)

        # 4. 粉丝采集 (从 erma0 借鉴的功能)
        if scrape_type == 'follower':
            _log(f"[SCRAPER] ===== 开始采集粉丝 =====")
            progress.update(current_type="粉丝", current_user=sec_uid[:12], progress=20)
            service = FollowerService(sec_uid, cookie_str)
            stats = await service.run(delay=delay, limit=limit)
            all_stats['follower'] = stats
            _log(f"[SCRAPER] 粉丝采集完成: {stats}")
            progress.update(progress=100, stats=all_stats)

        # 5. 喜欢采集 (从 erma0 借鉴的功能)
        if scrape_type == 'like':
            _log(f"[SCRAPER] ===== 开始采集喜欢 =====")
            progress.update(current_type="喜欢", current_user=sec_uid[:12], progress=20)
            service = LikeService(sec_uid, cookie_str)
            stats = await service.run(delay=delay, limit=limit)
            all_stats['like'] = stats
...
def main():
    parser = argparse.ArgumentParser(description="抖音数据采集（高级集成版）")
    ...
    parser.add_argument("--type", default="all", choices=["video", "comment", "reply", "all", "follower", "like"],
                        help="采集类型（默认 all）")
