...
class VideoService(BaseService):
    ...
    async def fetch(self, page_size: int = 30, delay: float = 1.0,
                    limit: int = 0, stop_at_id: str = None, **kwargs) -> List[Dict]:
        all_videos = []
        max_cursor = 0
        has_more = True
        retries = 0
        ...
        try:
            while has_more and retries < max_retries:
                ...
                try:
                    ...
                    if aweme_list:
                        # 增量判断：如果遇到已存在的 ID 且没有开启“全量覆盖”，则停止
                        if stop_at_id:
                            for i, video in enumerate(aweme_list):
                                if str(video.get("aweme_id")) == str(stop_at_id):
                                    logger.info(f"[采集] 触发增量停止：遇到已采集的作品 ID {stop_at_id}")
                                    aweme_list = aweme_list[:i]
                                    has_more = False
                                    break
    ...
    async def _fetch_incremental_limited(self, page_size: int, delay: float,
                                          limit: int, existing_ids: set) -> List[Dict]:
        """增量+限量模式：跳过已存在视频，持续翻页直到凑够 limit 个新视频或遍历完毕"""
        all_new_videos = []
        max_cursor = 0
        has_more = True
        retries = 0
        ...
        pbar = tqdm(desc="增量采集作品",
                    unit="条",
                    bar_format='{percentage:3.0f}% | {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]')

        try:
            while has_more and retries < max_retries:
                if len(all_new_videos) >= limit:
                    break
                try:
                    response = await self.api.fetch_videos(self.sec_uid, max_cursor, page_size)
                    aweme_list = response.get("aweme_list", [])
                    ...
                    if aweme_list:
                        for video in aweme_list:
                            ...
                            if aweme_id in existing_ids:
                                logger.debug(f"[采集] 跳过已存在视频 {aweme_id}")
                                continue
                        ...
                        if not has_more or len(all_new_videos) >= limit:
                            break
    ...
    async def run(self, delay: float = 1.0, limit: int = 0, incremental: bool = True, **kwargs) -> Dict:
        page_size = 18
        logger.info(f"[采集] 开始 用户={self.sec_uid} 类型=作品 模式={'增量' if incremental else '全量'}")
        ...
        if incremental and limit > 0:
            # 增量+指定数量：跳过已存在视频，持续翻页直到凑够 limit 个新视频或遍历完毕
            existing_ids = set(self.storage.get_video_ids(0))  # 所有已存储的视频 ID
            logger.info(f"[采集] 增量+限量模式：已存储 {len(existing_ids)} 个视频，目标新增 {limit} 个")
            raw_videos = await self._fetch_incremental_limited(
                page_size, delay, limit, existing_ids)
        else:
            # 无 limit 时使用经典增量 stop_at_id（首次遇到旧视频即停，效率高）
            stop_at_id = None
            if incremental:
                latest_id = self.storage.get_latest_video_id()
                if latest_id:
                    stop_at_id = latest_id
                    logger.info(f"[采集] 增量采集起始点 ID: {stop_at_id}")
            raw_videos = await self.fetch(page_size, delay, limit, stop_at_id=stop_at_id)
