...
class CommentService(BaseService):
    ...
    async def fetch(self, aweme_id: str, page_size: int = 50,
        ...
        while has_more:
            if limit > 0 and len(all_comments) >= limit:
                break
            try:
    ...
    async def run(self, aweme_ids: List[str] = None,
        ...
        # limit: 每个视频内最多采集多少评论（0=不限），由调用方决定
        video_limit = kwargs.get('video_limit', limit)
        if aweme_ids is None:
            aweme_ids = self.storage.get_video_ids(video_limit)
        ...
        for aweme_id in tqdm(aweme_ids, 
            ...
            try:
                raw_comments = await self.fetch(aweme_id, page_size, delay, limit=limit)
            except CookieExpiredError:
                raise
            except Exception as e:
