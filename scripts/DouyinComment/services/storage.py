...
class StorageManager:
    ...
    def get_latest_video_id(self) -> Optional[str]:
        """获取数据库或 CSV 中最新的作品 ID。"""
        if self.db:
            rows = self.db.query(
                f"SELECT {self.id_field} FROM {self.table_name} WHERE sec_uid = ? ORDER BY create_time DESC LIMIT 1",
                (self.sec_uid,)
            )
            if rows:
                return str(rows[0][self.id_field])
        ...
        if os.path.exists(filepath):
            try:
                with open(filepath, 'r', newline='', encoding='utf-8-sig') as f:
                    ...
                    rows = list(reader)
                    if rows:
                        # 注意：CSV 可能是追加写入的，不一定是按时间排序的，这里简单取最后一行
                        return rows[-1].get('aweme_id')
            except Exception:
                pass
        return None
    ...
    def _save_to_csv(self, items: List[Dict], aweme_id: str = None, video_timestamp: Union[int, float, None] = None) -> int:
        ...
        if not fields:
            logger.error(f"[存储] 字段配置为空，跳过写入（请检查 config.yaml 是否加载成功）→ {filepath}")
            return 0

        file_exists = os.path.exists(filepath) and os.path.getsize(filepath) > 0

        # 检测现有文件的标题行是否有效（防止config加载失败时写入空标题的情况）
        if file_exists and fields:
            try:
                with open(filepath, 'r', newline='', encoding='utf-8-sig') as f:
                    existing_fields = csv.DictReader(f).fieldnames
                if not existing_fields or all(not f.strip() for f in existing_fields):
                    # 标题行缺失或为空，重写文件
                    with open(filepath, 'r', newline='', encoding='utf-8-sig') as f:
                    ...
                    logger.warning(f"[存储] CSV 标题行缺失，已修复 → {filepath}")
                    self._csv_cache.pop(self.id_field, None)
                    file_exists = True
            except Exception as e:
                logger.warning(f"[存储] CSV 标题检测失败，将追加写入: {e}")
