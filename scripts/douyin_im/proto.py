...
from .utils import generate_millisecond, generate_req_sign, generate_webid


class ProtoBuilder:
    @staticmethod
    def build_normal_request(auth, cmd: int):
        request = RequestProto.Request()
        request.cmd = cmd
        request.sequence_id = random.randint(10000, 11000)
        request.sdk_version = "1.1.3"
        request.token = auth.ticket or ""
        request.refer = 3
        ...
        request.ts_sign = auth.ts_sign or ""
        request.sdk_cert = base64.b64encode((auth.client_cert or "").encode("utf-8")).decode("utf-8")
        return request

    @staticmethod
    def build_create_conversation_request(auth, to_user_id: int, my_user_id: int):
        request = ProtoBuilder.build_normal_request(auth, 609)
        request.body.create_conversation_v2_body.conversation_type = 1
        request.body.create_conversation_v2_body.participants.extend([int(to_user_id), int(my_user_id)])
        request_sign = generate_req_sign(
            {
                "sign_data": f"avatar_url=&idempotent_id=&name=&participants={to_user_id},{my_user_id}",
                "certType": "cookie",
                "scene": "web_protect",
            },
            auth.private_key,
        )
        request.reuqest_sign = request_sign
        return request

    @staticmethod
    def build_send_message_request(auth, conversation_id, conversation_short_id, ticket, message: str):
        ...
        request.body.send_message_body.ext.append(RequestProto.ExtValue(key="s:stime", value=str(generate_millisecond())))
        ...
        req_sign = generate_req_sign(
            {
                "sign_data": f'content={json.dumps(msg_content, separators=(",", ":"), ensure_ascii=False)}'
